import React from 'react';
import { ESSvgIcon, ESSvgIconProps } from '../../components/ESSvgIcon';

const BehaviorIcon = (props: ESSvgIconProps) => (
  <ESSvgIcon {...props}>
    <path d="M9.37 8.07l10 4a1 1 0 01.08 1.82l-3.7 1.86-1.85 3.7a1 1 0 01-.9.55h-.04a1 1 0 01-.89-.63l-4-10a1 1 0 011.3-1.3zM9.707 4.707A1 1 0 018 4V1a1 1 0 012 0v3a1 1 0 01-.293.707zM4.707 8.293A1 1 0 014 10H1a1 1 0 010-2h3a1 1 0 01.707.293zM3.707 2.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414-1.414l-2-2zM15.707 2.293a1 1 0 00-1.414 0l-2 2a1 1 0 001.414 1.414l2-2a1 1 0 000-1.414zM2.293 14.293l2-2a1 1 0 011.414 1.414l-2 2a1 1 0 01-1.414-1.414z" />
  </ESSvgIcon>
);

export default BehaviorIcon;
