...
import ESAppBar from '../src/components/ESAppBar/ESAppBar';
import { Box } from '@mui/material';
import NotificationAddIcon from '@mui/icons-material/NotificationAdd';
import OfficeSwitcher from '../src/components/OfficeSwitcher/OfficeSwitcher';
import initI18nInstance from '../src/services/i18n';
import { I18nProvider } from '@pankod/refine-core/src/contexts/translation/ITranslationContext';
import { TLocaleOptions } from '../src/types/locale';

export type EcosystemWrapperProps = Partial<EcosystemProps> & {
  customRoutes?: any[];
...
const Header = (props) => (
  <ESAppBar
    {...props}
    profileButton={<ProfileButton {...profileButtonProps} />}
    notifications={<MockNotifications />}
    searchField={<MockSearchField />}
    officeSwitcher={<OfficeSwitcher />}
  />
);

const EcosystemWrapper: React.FC<EcosystemWrapperProps> = ({
  children,
  theme: inTheme,
  dataProvider: inDataProvider,
  routerProvider: inRouterProvider,
  ...
  const dataProvider = React.useMemo(() => {
    return inDataProvider
      ? inDataProvider
      : {
          default: MockDataProvider(sources, 'http://localhost:3000/api'),
          auth: MockDataProvider(sources, 'http://localhost:3001/api'),
        };
  }, [inDataProvider]);

  const theme = React.useMemo(() => {
    return inTheme ? inTheme : LightTheme;
  }, [inTheme]);

  const routerProvider = React.useMemo(() => {
    return inRouterProvider
      ? inRouterProvider
      : {
  ...
  const i18n = useMemo(() => {
    return initI18nInstance(localeOptions);
  }, [localeOptions]);
